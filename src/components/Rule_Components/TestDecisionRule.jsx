...
const TestDecisionRule = ({
  ...
  const testRule = async () => {
    ...
    try {
      ...
      setIsTested(true);
      console.log("Setting testResult state with:", response.data);
      setTestResult(response.data);
      // Mark conditions as tested by clearing the modified flag
      console.log(
        "Test completed successfully, resetting conditionsModified flag"
      );
      sessionStorage.setItem("conditionsModified", "false");
      // Dispatch event to notify that conditions are no longer modified
      console.log("Dispatching conditionsModified event with detail: false");
      window.dispatchEvent(
        new CustomEvent("conditionsModified", { detail: false })
      );
      ...
      // Still mark as tested even though there was an error
      setIsTested(true);
      console.log("Setting testResult state with error:", {
        error: error.message || "An error occurred while testing the rule",
      });
      // Also reset conditionsModified flag
      sessionStorage.setItem("conditionsModified", "false");
      window.dispatchEvent(
        new CustomEvent("conditionsModified", { detail: false })
      );
      toast.error("Error testing rule, but marked as tested");
    } finally {
...
export default TestDecisionRule;
