...
const CreateDataSource = ({
  ...
  const [tableData, setTableData] = useState({ name: "", row_limit: "" });
  const dataDetails = useSelector((state) => state.dataSourceDetails);
  const resetData = { data: {}, isSelected: false };
  const platform = dataDetails?.data?.platform || "";
  const name = dataDetails?.data?.name || "";
  const connectionString = dataDetails?.data?.connectionString || "";
  const integration_id = dataDetails?.data?.integration_id || "";
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.post(
        "https://rule-engine-datasource-test.mfilterit.net/datasource/create",
        payload
      );
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-bold ml-4">Create Data-Source</h2>
          <button
            onClick={() => {
              dispatch(setDataSorceData(resetData));
              setIsOpen(false);
