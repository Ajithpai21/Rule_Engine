    ...
    const fetchExistingData = async () => {
      try {
        ...
        if (response.data.status === "Success") {
          ...
          // Set rule conditions first
          const rulesEnabled = response.data.data["rules enabled"];
          setCurrentConditions(rulesEnabled);

          // Use a timeout to ensure rule policy is set after the dropdown component has initialized
          if (response.data.data["Rule Policy"]) {
            setTimeout(() => {
              console.log(
                "Setting Rule Policy to:",
                response.data.data["Rule Policy"]
              );
              setSelectedRulePolicy(response.data.data["Rule Policy"]);
            }, 500); // Add a small delay to let component initialization complete
          }
        } else {
    ...
    >
      ...
      >
        <div className="topBar flex justify-between items-center">
          <div className="label text-2xl font-bold">Edit Rule Set</div>
