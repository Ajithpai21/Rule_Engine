...
const EditSimpleRule = ({ setOnClose }) => {
  ...
  const handleConditionChange = (conditionGroup) => {
    // Skip update if conditions aren't loaded yet
    if (isLoading) {
      return;
    }

    // Check if rules have been deleted, if so, prioritize this update
    const rulesCountChanged =
      currentConditions?.rules?.length !== conditionGroup?.rules?.length;

    // Skip only if structures are identical (and no rules were deleted)
    if (
      !rulesCountChanged &&
      JSON.stringify(currentConditions) === JSON.stringify(conditionGroup)
    ) {
      return;
    }

    console.log("Updating conditions from ConditionBuilder:", conditionGroup);
  ...
  const fetchRule = async () => {
    try {
      ...
      if (
        ...
        response.data.data.conditions.length > 0
      ) {
        // Create a deep copy to avoid reference issues
        const fetchedCondition = JSON.parse(
          JSON.stringify(response.data.data.conditions[0])
        );

        // Ensure the condition has the proper structure with at least one rule
        if (
          fetchedCondition &&
          fetchedCondition.type === "group" &&
          Array.isArray(fetchedCondition.rules)
        ) {
          // Make sure rules array is never empty to prevent component resets
          if (fetchedCondition.rules.length === 0) {
            fetchedCondition.rules = [
              {
                property: "",
                operator: "",
                value: "",
          ...
          console.log("Setting conditions from API:", fetchedCondition);
          ...
          if (response.data.data.result) {
            const transformedData = transformData(
              response.data.data.result.details
            );
          ...
          console.error(
            "Invalid condition structure in API response:",
            fetchedCondition
          );
          setIsLoading(false);
        }
      } else {
        console.error("No valid conditions in API response");
        setIsLoading(false);
      }
    } catch (error) {
    ...
    >
      ...
      >
        <div className="topBar flex justify-between items-center">
          <div className="label text-2xl font-bold">Edit Rule</div>
        ...
        <div className="mainBox flex h-full">
          ...
          <div className="condition w-[30%] h-full">
            ...
            <div className="btns w-full flex justify-end gap-4 h-16 p-2">
              ...
              >
                {isSaved ? "Update" : isSaving ? "Saving..." : "Save"}
              </button>
            </div>
          </div>
        </div>
